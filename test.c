...
void test_size() {
    ...
    // Perform enqueue and dequeue operations and check if size updates
    // correctly
    // ...
...
void test_waiting() {
    ...
    for (int i = 0; i < 3; i++) {
        thrd_create(&threads[i], (int (*)(void *))dequeue_with_wait,
                    &sleep_time);
    }
...
void test_enqueue_dequeue_with_sleep() {
    ...
    // Create multiple threads to concurrently dequeue items with a delay
    // between each dequeue operation
    thrd_t threads[3];
    for (int i = 0; i < 3; i++) {
...
int dequeue_with_sleep(void *arg) {
    int sleep_time =
        (rand() % 6 + 5) *
        1000; // Random sleep time between 5000 and 10000 milliseconds

    thrd_sleep(
        &(struct timespec){sleep_time / 1000, (sleep_time % 1000) * 1000000},
        NULL);
...
void test_fifo_order() {
    ...
    for (int i = 0; i < NUM_THREADS; i++) {
        ...
        thrd_sleep(&(const struct timespec){.tv_nsec = 5000000}, NULL);
    }
...
void test_edge_cases() {
    ...
    // int *item = (int *)dequeue();
    // assert(item == NULL);
    // printf("Dequeue from an empty queue - Assertion failed: Expected
    // NULL\n");
