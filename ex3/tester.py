...
SENDER_EXECUTABLE_PATH = "./message_sender"
READER_EXECUTABLE_PATH = "./message_reader"
...
def execute(command, exit_on_non_zero=True):
    ...
    if exit_on_non_zero and result.returncode != 0:
        ...
        exit(-1)
    return RunResult(
        result.returncode, result.stdout.decode("utf-8"), result.stderr.decode("utf-8")
    )
...
def create_slot_file(template, minor_num):
    ...
    if filename in FILE_NAMES:
        print(
            "THIS IS WRONG! Contact the developer of this file and tell him he made a mistake."
        )
        delete_all_files()
        exit(-1)
    execute(
        f"test ! -f {filename} && sudo mknod {filename} c 235 {minor_num} || echo '' > /dev/null"
    )
...
def write_to_file(filename, channel_id, msg, **kwargs):
    ...
    return execute(
        f"{SENDER_EXECUTABLE_PATH} {filename} {channel_id} '{msg}'", **kwargs
    )
...
def assert_contains(containing, contained, extra_info={}):
    if contained not in containing:
        ...
        print(
            f"to be contained in: {Back.YELLOW}{Fore.BLACK}{containing}{Style.RESET_ALL}"
        )
        extra_info_and_exit(extra_info)
...
def read_from_file(filename, channel_id):
    return execute(
        f"{READER_EXECUTABLE_PATH} {filename} {channel_id}", exit_on_non_zero=False
    )
...
def test_ioctl_zero_fails():
    ...
    for filename in FILE_NAMES:
        ...
        assert_contains(result.stderr, "Invalid argument", {"filename": filename})
...
def test_long_write_fails():
    ...
    assert_contains(
        result.stderr,
        "Message too long",
        {"filename": filename, "message_length": size, "execution result": result},
    )
