...
void read_no_message(int fd) {
    ...
    if (rc == -1) {
        if (errno != EWOULDBLOCK) {
            ...
            fprintf(
                stderr,
                "read_no_message: read failed with wrong type of error: %d\n",
                errno);
            fprintf(stderr,
                    "read_no_message: errno should be EWOULDBLOCK (11)\n");
        }
    } else {
        passed = 0;
        fprintf(
            stderr,
            "read_no_message: read hasn't failed although he should have\n");
    }
    if (passed) {
...
void write_read_null(int fd) {
    ...
    if (rc != -1) {
        ...
        fprintf(
            stderr,
            "write_read_null: write hasn't failed although he should have\n");
    }
    ...
    if (rc != -1) {
        ...
        fprintf(
            stderr,
            "write_read_null: read hasn't failed although he should have\n");
    }
    if (passed) {
...
void error_buffer_size(int fd) {
    ...
    fflush(stdout);
    int passed = 1;
    char *long_bffr = "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"
                      "aaaaaaaaaaaaaaaaaaa"
    ...
    if (rc == -1) {
        if (errno != EMSGSIZE) {
            passed = 0;
            fprintf(stderr,
                    "error_buffer_size: write failed with wrong type of error: "
                    "%d\n",
                    errno);
            fprintf(stderr,
                    "error_buffer_size: errno should be EMSGSIZE (122)\n");
        }
    } else {
        passed = 0;
        fprintf(
            stderr,
            "error_buffer_size: write hasn't failed although he should have\n");
    }
    rc = write(fd, bffr, 37);
    if (rc == -1) {
        passed = 0;
    ...
    if (rc == -1) {
        if (errno != ENOSPC) {
            ...
            fprintf(
                stderr,
                "error_buffer_size: read failed with wrong type of error: %d\n",
                errno);
        ...
        fprintf(
            stderr,
            "error_buffer_size: read hasn't failed although he should have\n");
    }
    if (passed) {
...
void write_read_before_ioctl(int fd) {
    ...
    if (rc == -1) {
        if (errno != EINVAL) {
            passed = 0;
            fprintf(stderr,
                    "write_read_before_ioctl: write failed with wrong type of "
                    "error: %d\n",
                    errno);
            fprintf(stderr,
                    "write_read_before_ioctl: errno should be EINVAL (22)\n");
        }
    } else {
        passed = 0;
        fprintf(stderr, "write_read_before_ioctl: write hasn't failed although "
                        "he should have\n");
    }
    ...
    if (rc == -1) {
        if (errno != EINVAL) {
            passed = 0;
            fprintf(stderr,
                    "write_read_before_ioctl: read failed with wrong type of "
                    "error: %d\n",
                    errno);
            fprintf(stderr,
                    "write_read_before_ioctl: errno should be EINVAL (22)\n");
        ...
        passed = 0;
        fprintf(stderr, "write_read_before_ioctl: read hasn't failed although "
                        "he should have\n");
    }
    rc = ioctl(fd, MSG_SLOT_CHANNEL, 10);
    if (rc == -1) {
        fprintf(stderr,
                "write_read_before_ioctl: ioctl failed with error: %d\n",
                errno);
        return;
    }
    rc = write(fd, bffr, 3);
    if (rc == -1) {
        passed = 0;
        fprintf(stderr,
                "write_read_before_ioctl: write failed with error: %d\n",
                errno);
    }
    rc = read(fd, bffr_read, 3);
    if (rc == -1) {
    ...
    if (strcmp(bffr, bffr_read) != 0) {
        passed = 0;
        fprintf(stderr, "write_read_before_ioctl: read value doesnt match "
                        "written message\n");
    }
    if (passed) {
