...
int perform_random_write(int fd) {
    ...
    if (rc == -1) {
        if (channel > 0) {
            fprintf(stderr,
                    "perform_random_write: ioctl failed with error: %d\n",
                    errno);
            fprintf(stderr,
                    "DETAILS:\n"
                    "fd is - %d,\n"
        ...
        if (DEBUG) {
            fprintf(stderr, "perform_random_write: PASSED!\n");
        }
        return 0;
    }
    ...
    if (length > 0 && length <= BUFF_SIZE) {
        if (rc == -1) {
            passed = 0;
            fprintf(stderr,
                    "perform_random_write: write failed with error: %d\n",
                    errno);
            fprintf(stderr,
                    "DETAILS:\n"
                    "fd is - %d,\n"
        ...
        if (rc != -1) {
            passed = 0;
            fprintf(stderr, "perform_random_write: write hasn't failed "
...
int perform_random_read(int fd) {
    ...
    if (rc == -1) {
        if (channel > 0) {
            fprintf(stderr,
                    "perform_random_read: ioctl failed with error: %d\n",
                    errno);
            fprintf(stderr,
                    "DETAILS:\n"
                    "fd is - %d,\n"
        ...
        if (DEBUG) {
            fprintf(stderr, "perform_random_read: PASSED!\n");
        }
        return 0;
    }
    ...
    if (rc == -1) {
        if (channels_msg[channel] == 0) {
            if (errno != EWOULDBLOCK) {
                passed = 0;
                fprintf(stderr,
                        "perform_random_read: read failed with wrong type of "
                        "error: %d\n",
                        errno);
                fprintf(
                    stderr,
                    "perform_random_read: errno should be EWOULDBLOCK (11)\n");
            }
        } else {
            passed = 0;
    ...
    if (-1 != rc && strcmp(bffr, channels_msg[channel]) != 0) {
        ...
        fprintf(
            stderr,
            "perform_random_read: read value doesnt match written message\n");
        fprintf(stderr,
                "DETAILS:\n"
                "fd is - %d,\n"
                "channel ID is - %d,\n"
